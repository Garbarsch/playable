...
const markers: Marker[] = [
  { value: 20, label: '20%' },
  { value: 40, label: '40%' },
  { value: 60, label: 'Peer Average (60%)' },
  { value: 80, label: '80%' },
  { value: 100, label: '100%' },
];

export default function ProductivityMeter({ value }: { value: number }) {
  return (
    <div className="relative w-full bg-gray-200 rounded-lg h-4 overflow-visible">
      <div
        style={{ width: `${value}%` }}
        className="bg-green-500 h-4 rounded-lg transition-all duration-500 ease-out"
      ...
      {markers.map(({ value: mark, label }) => (
        <div
          key={mark}
          className="absolute -top-7 flex flex-col items-center -translate-x-1/2 pointer-events-none"
          style={{ left: `${mark}%` }}
        >
